...
extension UIViewController {
        ...
        if let window = UIApplication.shared.keyWindow {
            ...
            if animation == .fade {
                ...
                if position == .bottom {
                    ...
                    self.view.frame = CGRect(x: 0.0, y: r.size.height - self.view.frame.size.height - bottomPadding, width: size.width, height: size.height)
                }
    ...
    func showSnackBarBottom(position: SnackBarPosition = .bottom, duration: TimeInterval = 3.0, animation: SnackBarAnimation = .slide, avoidTabBar: Bool = false, didFinish: (() -> Void)? = nil) {
        if let window = UIApplication.shared.keyWindow {
            let r = UIScreen.main.bounds
            ...
            if animation == .slide {
                ...
                UIView.animate(withDuration: 0.2, delay: 0.0, options: .curveEaseIn, animations: {
                    if position == .top {
                        ...
                        if let window = UIApplication.shared.keyWindow {
                            bottomPadding = window.safeAreaInsets.bottom + 24
                            if avoidTabBar {
                                bottomPadding = window.safeAreaInsets.bottom + 64
                            }
                        }
                        self.view.frame = CGRect(x: 0.0, y: r.size.height - self.view.frame.size.height - bottomPadding, width: self.view.frame.size.width, height: self.view.frame.size.height)
                    }
